<template>
  <transition name="fade-overlay">
    <div v-show="showModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50"
      ...
      <div class="w-full max-w-lg px-4 sm:px-0">
        ...
        <transition name="fade-modal-up" mode="out-in">
          ...
          <!-- Loading State -->
          <div v-if="loading" key="loading-state" class="flex items-center justify-center w-full min-h-[300px]">
            <BaseLoadingSpinner />
          </div>

          <!-- Actual Modal Content -->
          <div v-else key="content-state"
            ...
            <header class="flex justify-between items-center mb-6">
              ...
              <button @click="$emit('close')" class="text-white opacity-75 hover:opacity-100 transition-opacity">
            ...
            <div class="space-y-6">
              ...
              <div class="w-full">
                ...
                <div class="relative">
                  <input type="email" v-model="form.email" @blur="validateEmail" @input="validateEmail"
                    placeholder="example@email.com" class="w-full rounded-md border px-3 py-2 pr-12 text-sm
             bg-transparent text-white
             border-white/60 placeholder-white/40
             focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-500" />

                  <!-- ICON -->
                  <i
                    class="mdi mdi-email-edit-outline absolute inset-y-0 right-3 flex items-center text-white text-xl pointer-events-none"></i>
                </div>
                ...
                <BaseDropdown v-model="isInviteOpen" close-on-click class="w-full">
                  <template #trigger>
                    <button type="button"
                      ...
                      <i class="mdi mdi-chevron-down transition-transform duration-300" :class="{
                  ...
                  <div class="py-1 bg-white/95 rounded-md shadow">
                    <button v-for="option in InviteOption" :key="option.value" type="button"
...
<button
  ...
  class="
    w-full
    bg-linear-to-tr from-[#1C244B] to-[#682DB5]
    hover:from-[#2A3570] hover:to-[#8A4BE3]
    active:from-[#4354b7] active:to-[#7f23ff]
...
const InviteOption: InviteOption[] = [
  ...
  { label: 'ผู้เยี่ยมชม', value: '(view)' },
  { label: 'ผู้ตรวจสอบ', value: '(auditor)' },
]

const isInviteOpen = ref(false)
const selectedInviteValue = ref<string>('member')
